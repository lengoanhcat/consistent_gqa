...
def parseArgs():
    ...
    parser.add_argument("--hvdspeed",       action = "store_true",          help = "enable horovod parallelism")
    parser.add_argument("--hvdoptim",       default = "average", type = str,help = "operation between gradients of different hvd nodes")
    ...
    parser.add_argument("--numSamples",      default = 100, type=int, help = "100,300,700,1100, ... other prime numbers")
    parser.add_argument("--datapct",         default = 100, type=int, help = "percentage of used data")
    ...
    parser.add_argument("--checkTrain",     action = "store_true",      help = "check average logic loss given task info on training set")
    parser.add_argument("--checkVal",       action = "store_true",      help = "check average logic loss given task info on validation set")
    ...
    parser.add_argument("--batchStyle",     default = 'original', type = str, help ="different ways of organising data into batches original/family/hybrid")
    parser.add_argument("--familyStyle",    default = 'unique', type = str, help="unique / ununique family of questions")
    parser.add_argument("--cutOffEpoch",    default = 24, type = int, help = "cutoff Epoch for hybrid to switch from original to family")
    parser.add_argument("--minNoQuest",     default = 10, type = int,       help = "minimum # questions in a family batch")
    ...
    parser.add_argument("--questBN",    action = "store_true",              help = "use batch normalization in the question unit (control)")
    ...
    # parser.add_argument("--useLogicConstr", action = "store_true", help = "flag for using logic constraint")
    parser.add_argument("--tflSS", action = "store_true", help = "use t-norm logic SS family")
    parser.add_argument("--tflFK", action = "store_true", help = "use t-norm logic Frank family")
    parser.add_argument("--tflLambda", default = .0, type=float, help = "parameters of tfl logic SS / Frank ")
    parser.add_argument("--tflLossWeight", default = 1e-6, type=float, help = "parameters of tfl logic SS / Frank")
    parser.add_argument("--tflRules", default = 'B', type=str, help = "(B)inary/(G)lobal/(A)ttr/(O)ject/(R)elation/(E)ntailement")
    parser.add_argument("--tflWeightMode", default = 'fw', type=str, help = "using fixed weight / softmax weight")
    parser.add_argument("--tflTaskWeight", default = 1.0, type=float, help = "parameters of task prediction")
    parser.add_argument("--useDLM", action = "store_true", help = "use exponential model to optimizer constraint weights")
    parser.add_argument("--dlmAlpha", default = 1.0, type=float, help="network regularization parameter of DLM")
    parser.add_argument("--dlmBeta", default = 1.0, type=float, help="constraint regularization parameter of DLM")
    ...
    parser.add_argument("--outAnsClassifierDims",   default = [512], nargs = "*",   type = int, help = "dimensions of the classifier for answers")
    parser.add_argument("--outTskClassifierDims",   default = [16], nargs = "*", type = int, help = "dimensions of the classifier for tasks")
...
def configGQA():
    ...
    config.wordVectorsFile = "{dataBasedir}/data/glove.6B.{dim}d.txt".format(dataBasedir=config.dataBasedir,dim = config.wrdQEmbDim) #
    config.wordVectorsSemanticFile = "{dataBasedir}/data/glove.6B.{dim}d.txt".format(dataBasedir=config.dataBasedir,dim = config.semanticWordsEmbDim) #
