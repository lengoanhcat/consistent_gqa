...
def linear(inp, inDim, outDim, dropout = 1.0,
    ...
    with tf.variable_scope("linearLayer" + name, reuse = reuse):
        ...
        if batchNorm is not None:
            ...
            inp = tf.compat.v1.layers.batch_normalization(
                inp,
                momentum = batchNorm["decay"],
                fused=False,
                center = batchNorm["center"],
                scale = batchNorm["scale"],
                training = batchNorm["train"],
                renorm_clipping={
                    'rmax':batchNorm['rmax'],
                    'rmin':batchNorm['rmin'],
                    'dmax':batchNorm['dmax']},
                renorm=True)
            '''
            iSyncBN = hvd.SyncBatchNormalization(
                axis=-1,
                momentum = batchNorm["decay"],
                fused=False,
                center = batchNorm["center"],
                ...
                renorm_clipping={
                    ...
                    'dmax':batchNorm['dmax']},
                renorm=True)
            iSyncBN.apply(inp, training = batchNorm["train"])
