    ...
    def write(self, memory, info, control, contControl = None, name = "", reuse = None):
        with tf.variable_scope("write" + name, reuse = reuse):
            ...
            if config.memoryBN:
                ...
                '''
                # batch norm in legacy layer
                '''
                newMemory = tf.compat.v1.layers.batch_normalization(
                    newMemory,
                    momentum = config.bnDecay,
                    fused=False,
                    ...
                    renorm=True)
                '''
                mSyncBN = hvd.SyncBatchNormalization(
                    momentum = config.bnDecay,
                    fused=False,
                    center = config.bnCenter,
                    scale = config.bnScale,
