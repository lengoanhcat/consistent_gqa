...
from macnetwork.program_translator import ProgramTranslator
...
class Preprocesser(object):
    ...
    def writePreds(self, res, tier, suffix = ""):
        if res is None:
            return
        if 'preds' in res.keys():
            preds = res["preds"]
        elif 'answerPreds' in res.keys():
            preds = res["answerPreds"]
    ...
    def vectorizeData(self, data):
        ...
        if "task" in data[0].keys():
            taskLabels = np.array([d["task"] for d in data])
            vectorizedData["tasks"] = taskLabels
