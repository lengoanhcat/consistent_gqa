...
class AtomTensor(FormulaTensor):
    ...
    def apply_op(self,args):
        if self._tensor is None:
            ...
            self._tensor = t
        tensor = tf.reshape(self._tensor, tf.stack([v.domain.size for v in args]))
    ...
    for i,v in enumerate(sorted_vars):
        for j, arg in enumerate(args):
            if v not in arg.variables_list:
                ...
                if len(args)>1:
                    if np.isscalar(v.domain.size):
                        multiplies = np.ones_like(temp.get_shape())
                        multiplies[i] = v.domain.size
                        temp = tf.tile(temp, multiplies)
                    elif v.domain.size.op.type == 'Placeholder':
                        temp_shape = list(np.ones_like(temp.get_shape()))
                        temp_shape[i] = v.domain.size
                        multiplies = tf.stack(temp_shape)
                        temp = tf.tile(temp, multiplies)
