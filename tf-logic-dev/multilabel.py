...
minibatch_size_ph = tf.placeholder(tf.int64)
images = tfl.Domain("Images", data=x_final, size=minibatch_size_ph)
...
constraints.append(tfl.constraint("forall x: (not flies(x)) or (not mammal(x)) <-> cat(x) or dog(x) or horse(x) or deer(x) or truck(x) or ship(x) or automobile(x) or frog(x)"))
...
feed_dict = {contr_weight: 0.001, minibatch_size_ph: minibatch_size}
